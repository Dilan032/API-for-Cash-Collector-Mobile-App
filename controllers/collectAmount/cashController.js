...
//     const customerDetails = req.body; // Assume this is an array of customer objects

//     try {
//         for (const customer of customerDetails) {
//             const { AccNo, Bal, EmpCode, DailyTotal } = customer;
//             const currentDate = new Date();
//             const LastTraDat = currentDate.toISOString().split('T')[0]; // 'YYYY-MM-DD'

//             // Update `pltra` table
//             const updatePltra = await new Promise((resolve, reject) => {
//                 db.query(
//                     'UPDATE pltra SET Bal = ?, EmpCode = ? WHERE placc_AccNo = ?',
//                     [Bal, EmpCode, AccNo],
//                     (error, result) => {
//                         if (error) return reject(error);
//                         if (result.affectedRows === 0) {
//                             return reject(new Error(`Account ${AccNo} not found in pltra`));
//                         }
//                         resolve(result);
//                     }
//                 );
...
//                 db.query(
//                     'UPDATE placc SET Bal = ?, LastTraDat = ?, DailyTotal = ? WHERE AccNo = ?',
//                     [Bal, LastTraDat, DailyTotal, AccNo],
//                     (error, result) => {
//                         if (error) return reject(error);
//                         if (result.affectedRows === 0) {
//                             return reject(new Error(`Account ${AccNo} not found in placc`));
//                         }
//                         resolve(result);
//                     }
//                 );
